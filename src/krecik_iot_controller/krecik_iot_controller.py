...
class KrecikIOTController:
    ...
    def _send_data_to_server(self, data_dict, verbose=True):
        ...
        message_encrypted = encrypt_aes256(
            self.datasource.get_aes_key(), self.iv, json.dumps(data_dict))

        payload = {"device_id": self.auth_key,
    ...
    def _activate_device_in_server(self, verbose=True):
        url = self.__get_backend_url("/devices/"+self.auth_key+"/activate")
        message = encrypt_aes256(self.datasource.get_aes_key(
        ), self.iv, json.dumps({"serial_number": self.device_id}))
        payload = {"encrypted_message": message}
        data = json.loads(json.dumps(payload))
        try:
    ...
    def _read_and_send_data(self, sleep_interval_s):
        while not self.stop_sending_data:
            data = self.sensor_service.get_data()
    ...
    def run(self, sleep_interval_s):
        self.ble_server.set_message("R")
        sender_thread = threading.Thread(
            target=self._read_and_send_data, args=[sleep_interval_s])
        sender_thread.start()
        ...
        try:
            while True:
                ...
                try:
                    data_from_phone = self.ble_server.get_data()
                    if self.datasource.check_if_valid(data_from_phone):
                        print("Received wifi update Connecting...")
                        self.reinitiate_wifi()
                    print("Received data... Success!")

                except RuntimeError as e:
                    if str(e)[1] == ':':
                        print("Received data...", end=" ")
                        self.ble_server.set_message(str(e)[0])
                        print(str(e)[3:])
                    sleep(1)

        except KeyboardInterrupt:
            self.stop_sending_data = True
            return
